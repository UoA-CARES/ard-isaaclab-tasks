...
"""State-based Shadow Hand cube-repose environment.

Self-contained migration of the official ``Isaac-Repose-Cube-Shadow-Direct-v0``
benchmark (IsaacLab 2.3.X). All env machinery is defined here so this single file
is the complete environment and the ARD reward edit target: ``_get_rewards`` is
the only method ARD rewrites (see ``ard_meta.yaml``). Everything else — scene,
observations, resets, success/fitness metrics — is unchanged from the official
source.
"""
